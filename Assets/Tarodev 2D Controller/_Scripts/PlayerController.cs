...
namespace TarodevController {
    ...
    public class PlayerController : MonoBehaviour, IPlayerController {
        ...
        public Vector2 MouseInput => _frameInput.Mouse;
        ...
        protected virtual bool IsPositionClear(Vector2 pos) {
            Physics2D.queriesHitTriggers = false;
            var hit = Physics2D.OverlapCapsule(pos, _standingCollider.size, _standingCollider.direction, 0, ~_stats.PlayerLayer);
            Physics2D.queriesHitTriggers = _cachedTriggerSetting;
            return !hit;
        }
        ...
        protected virtual void HandleWalls() {
            ...
            if (_wallHits[0]) _wallHits[0].GetContacts(_wallContact); // for tilemap compatibility
            WallDirection = _wallHitCount > 0 ? (int)Mathf.Sign(_wallContact[0].point.x - transform.position.x) : 0;

            if (!_isOnWall && ShouldStickToWall() && _rb.velocity.y <= 0) SetOnWall(true);
        ...
        protected virtual void HandleLedgeGrabbing() {
            ...
            if (yInput != 0 && (xInput == 0 || Mathf.Sign(xInput) == WallDirection) && _hasControl) {
            ...
            // if (!IsPositionClear(finalPos)) return;
        ...
        protected virtual IEnumerator ClimbLedge() {
            ...
            var lockedUntil = Time.time + _stats.LedgeClimbDuration;
            ...
            targetPos = _ledgeCornerPos + Vector2.Scale(_stats.StandUpOffset, new(WallDirection, 1f));
            transform.position = targetPos;
        ...
        protected virtual void HandleLadders() {
            ...
            if (!ClimbingLadder && CanEnterLadder && MountLadderInputReached) ToggleClimbingLadder(true);
            else if (ClimbingLadder && (_ladderHitCount == 0 || DismountLadderInputReached)) ToggleClimbingLadder(false);

            // Snap to center of ladder
            if (ClimbingLadder && _frameInput.Move.x == 0 && _stats.SnapToLadders && _hasControl) {
                var pos = _rb.position;

                // For TileMap compatibility (kind of hacky)
                // We can't use GetContacts because ladder colliders are set as Triggers
                // This assumes many things: Rectangular grid, cell size of 1xN, and ladder is 1 cell width
                // TODO: If the player's center is outside the ladder, but collider is overlapping the tile, this pushes the player out
                var targetX = _ladderHits[0] is TilemapCollider2D ? (int)transform.position.x + 0.5f : _ladderHits[0].transform.position.x;
                _rb.position = Vector2.SmoothDamp(pos, new Vector2(targetX, pos.y), ref _ladderSnapVel, _stats.LadderSnapSpeed);
            }
        }
        ...
        private void ToggleClimbingLadder(bool on) {
            ...
            else if (_ladderHitCount > 0)
                _frameLeftLadder = _fixedFrame; // for jumping to prevent immediately re-mounting ladder
        ...
        protected virtual void ToggleCrouching(bool shouldCrouch) {
            ...
            if (Crouching && !IsPositionClear(pos)) return; // Prevent standing into colliders
        ...
        protected virtual void WallJump() {
            ...
            _speed = Vector2.Scale(_stats.WallJumpPower, new(-WallDirection, 1));
            Jumped?.Invoke(true);
        }
        ...
        protected virtual void HandleHorizontal() {
            ...
            // Crawling
                ...
                var xInput = Mathf.Sign(_frameInput.Move.x) * _frameInput.Move.magnitude; // no longer punished for pressing down due to input being normalized
                ...
                _speed.x = Mathf.MoveTowards(_speed.x, xInput * diminishedMaxSpeed, _stats.GroundDeceleration * Time.fixedDeltaTime);
            }
        ...
        private void OnValidate() {
            ...
            if (_rb == null && !TryGetComponent(out _rb)) Debug.LogWarning("Ensure the GameObject with the Player Controller has a Rigidbody2D", this); // serialized but hidden in the inspector
        }
#endif
    }
